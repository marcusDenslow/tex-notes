...
\\
\begin{align*}
    \vec{\nabla} \cdot \left( \frac{1}{\sqrt{V} } \sum_{k} \sum_{\alpha=1}^{2} \hat{\epsilon}^{\left( \alpha \right) }  \right) 
.\end{align*}

pengene bare flyr ut av lomma
oslo er dyrt ass. har leilighet der. knull meg baklengs god damn. var heldig da fordi grandma og grandpa betalte for en leilighet men må betale alt annet selv. nudler 1 gang om dagen thats it. 

this is latex, can you not read????????????????????????????????????????????
