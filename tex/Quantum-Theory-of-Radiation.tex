...
\begin{align*}
	\Box A_{\mu} &= 0 \\
	\Box \left( \frac{1}{\sqrt{V} } \sum_{k} \sum_{\alpha=1}^{2} \epsilon^{\left( \alpha \right) }_{\mu} c_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} + c.c. \right) &= \frac{1}{\sqrt{V} } \sum_{k} \sum_{\alpha=1}^{2} \epsilon^{\left( \alpha \right) } c_{k, \alpha} \left( 0 \right)  \Box e^{ik_{p}x_{p}} + c.c.
.\end{align*}




yeah they are some smart people out there. 
