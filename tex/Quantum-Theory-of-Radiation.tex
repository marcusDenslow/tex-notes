...
\\
\begin{align*}
	A_{\mu} = \frac{1}{\sqrt{V} } \sum_{k} \sum_{\alpha= 1}^{2} \hat{\epsilon}^{\left( \alpha \right) }_{\mu} \left(  c_{k, \alpha}  \left( 0 \right)  e^{ik_{p} x_{p}} + c^{*}_{k, \alpha} \left( 0 \right) e^{- ik_{p} x_{p}} \right) 
.\end{align*}
