...
\\
\begin{align*}
	\vec{\nabla} \cdot \left( \frac{1}{\sqrt{V} } \sum_{k} \sum_{\alpha=1}^{2} \hat{\epsilon}^{\left( \alpha \right) }  \right)  % this one is not done brodettes
.\end{align*}


ja FAKTYISDK, 1 gang. av en random ass kar. og noen jenter på et slipp engang hata som faen in my face. hun var sånn fuckings 09 på slip. JEG SVERGEr. det var slipp UTE og de sjekka ikke legg. noen av de jentene jeg sverger var født på sykehuset par timer før slippet. kasta inn på slipp day 1.

hva er det de kommenterer på??
