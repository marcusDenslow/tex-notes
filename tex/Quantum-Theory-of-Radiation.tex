...
\end{equation}
Observables corresponding to operators which do not commute have an uncertainty principle between them. So we can't fix the number of photons and know the fields exactly. Fluctuations in the field take place even in the vacuum state, where we know there are no photons.
\\
\\
Of course the average value of the Electric or Magnetic field vector is zero by symmetry. To get an idea about the size of the field fluctuations, we should look at the mean square value of the field, for example in the vacuum state. We compute $\displaystyle \left<\left| \vec{E} \cdot \vec{E} \right| 0 \right>$.
\\
\\
\begin{align*}
	\vec{E} &= - \frac{1}{c} \frac{ \partial \vec{A} }{ \partial t }  \\
	A_{\mu} &= \frac{1}{\sqrt{V} } \sum_{k, \alpha}  \sqrt{\frac{\hbar c^2}{2 \omega}} \epsilon^{\left( \alpha \right) }_{\mu} \left( a_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} 0 a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \right) \\
	\vec{A} &= \frac{1}{\sqrt{V} } \sum_{k, \alpha} \sqrt{\frac{\hbar c^2}{2 \omega}} \hat{\epsilon}^{\left( \alpha \right) } \left( a_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} + a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \right) \\
	\vec{E} &= -i \frac{1}{c} \frac{1}{\sqrt{V} } \sum_{k, \alpha} \sqrt{\frac{\hbar c^2}{2\omega}}  \hat{\epsilon}^{\left( \alpha \right) } \left( -\omega a_{k, \alpha} \left( 0 \right)  \right) e^{ik_{p}x_{p}} + \omega a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \\
	\vec{E} &= \frac{i}{\sqrt{V} } \sum_{k, \alpha} \sqrt{\frac{\hbar \omega}{2}} \hat{\epsilon}^{\left( \alpha \right) } \left( a_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} -a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \right) \\
	\vec{E} \left|0 \right> &= \frac{i}{\sqrt{V} } \sum_{k, \alpha} \sqrt{\frac{\hbar \omega}{2}} \hat{\epsilon}^{\left( \alpha \right) } \left( -a^{\dagger}_{k, \alpha} e^{-ik_{p}x_{p}} \right) \left|0 \right> \\
	\left<0 \left| \vec{E} \cdot \vec{E} \right| 0 \right> &= \frac{1}{v} \sum_{k, \alpha} \frac{\hbar \omega}{2} 1 \\
	\left<0 \left| \vec{E} \cdot \vec{E} \right| 0 \right> &= \frac{1}{V} \sum_{k} \hbar \omega \to \infty
.\end{align*}
\\
\\
(Notice that we are basically taking the absolute square of $\displaystyle \vec{E} \left|0 \right>$ and that the orthogonality of the states collapses the result down to a single sum.)
\\
\\
The calculation is illustrative even though the answer is infinite. Basically, a term proportial to $\displaystyle aa^{\dagger}$ first creates one photon absorbs it giving a nonzero contribution for every oscillator mode. The terms sum to infinity but really it is the infinitesimally short wavelengths that cause this. Again, some cut off in the maximum energy would make sense.
\\
\\
The effect of these field fluctuations on particles is mitigated by quantum mechanics. In reality, any quantum particle will be spread out over a finite volume and it's the average field over the volume that might cause the particle to experience a force. So we could average the Electric field over a volume, then take the mean square of the average. If we average over a cubic volume $\Delta V = \Delta l^3$, when we find that.
\\
\\
\begin{equation}
    \left<0 \left| \vec{E} \cdot \vec{e} \right| 0 \right> \approx \frac{\hbar c}{\Delta l^{4}}
\end{equation}
\\
\\
...
\\
\begin{equation}
	\left[ E_{x} \left( x \right) B_{y} \left( x' \right)  \right] = i c \hbar \delta \left( ds = \sqrt{\left( x-x' \right)_{p} \left( x-x' \right)_{p}  }  \right) 
\end{equation}
\\
\\
There is a nonzero commutator of the two spacetime points are connected by a light-like vector. Another way to say this is that the commutor is non-zero if the coordinates are simultaneous. This is a reasonable result considering causality.
\\
\\
To make a narrow beam of light, one must adjust the phases of various components of the beam carefully. Another version ofg the uncertainty relation is that $\Delta N \Delta \phi \ge 1$, where $\phi$ is the phase of the Fourier component and $N$ is the number of photons.
\\
\\
Of course the Electromagnetic waves of classical physics usually have large numbers of photons and the quantum effects are not apparent. A good condition to identify the boundary between classical and quantum behavior is that for the classical E\&M to be correct the number of photons per cubic wavelength should be much greater than 1.
\\
\\
\section{Emission and Absorption of Photons by Atoms} 
The interaction of an electron with the quantized field is already in the standard Hamiltonian
\begin{align*}
	\mathbf{H} &= \frac{1}{2m} \left( \vec{p} + \frac{e}{c} \vec{A} \right)^2 + V(r) \\
	\mathbf{H_{int}} &= - \frac{e}{2mc} \left( \vec{p} \cdot \vec{A} + \vec{A} \cdot \vec{p} \right)  + \frac{e^2}{2mc^2} \vec{A} \cdot \vec{A} \\
	&= - \frac{e}{mc} \vec{A} \cdot \vec{p} + \frac{e^2}{2mc^2} \vec{A} \cdot \vec{A}
.\end{align*}
\\
\\
For completeness we should add the interaction with the spin of the electron $\displaystyle \mathbf{H} = - \vec{\mu} \cdot \vec{B}$
\\
\\
\begin{equation}
    \boxed{\mathbf{H_{int}} = -\frac{e}{mc} \vec{A} \cdot \vec{p} + \frac{e^2}{2mc^2} \vec{A} \cdot \vec{A} - \frac{e \hbar}{2mc} \vec{\sigma} \cdot \vec{\nabla} \times \vec{A}  } 
\end{equation}
\\
\\
For an atom with many electrons, we must sum over all the electrons. The field is evaluated at the coordinate $x$ which should be that of the electron. This interaction Hamiltonian contains operators to create and annihilate photons with transitions between atomic states. From our previous study of time dependant perturbation theory, we know that transitions between initial and final states are proporional to the matrix element of the perturbing Hamiltonian between states, $\displaystyle \left<n \left| \mathbf{H_{int}} \right|i  \right>$. The initial state $\displaystyle \left|i \right>$ should include a direct product of the atomic state and the photon state. Lets concentrate on one type of photon for now. We then could write
\\
\\
\begin{equation}
    \left|i \right> = \left| \psi_{i}; \vec{n}_{k, \alpha} \right>
\end{equation}
\\
\\
...
\\
\\
We will first consider the absorption of the one photon from the field. Assume there are $\displaystyle n_{\vec{k}, \alpha}$ photons of this type in the initial state and that one photon is absorbed. We therefore will need a term in the interaction Hamiltonian that contains on annihitation operators (only). This will just come from the linear term in A.
\\
\\
\begin{align*}
	\left<n \left| \mathbf{H}_{int} \right|i  \right> &= \left<\psi_{n}; n_{\vec{k}, \alpha} -1 \left| - \frac{e}{mc} \vec{A} \cdot \vec{p} \right| \psi_{i}; n_{\vec{k}, \alpha}  \right> \\
	&= -\frac{e}{mc} \left<\psi_{n}; n_{\vec{k}, \alpha} -1 \left| \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar c^2}{2 \omega}} \hat{\epsilon}^{\left( \alpha \right) } \left( a_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} + a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \right) \cdot \vec{p}    \right| \psi_{i}; n_{\vec{k}, \alpha}  \right> \\
	\left<n \left| \mathbf{H}^{\left( abs \right) }_{int}  \right|i  \right> &= -\frac{e}{mc} \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar c^2}{2\omega}}  \left<\psi_{n}; n_{\vec{k}, \alpha} -1 \left| \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p} \left( a_{k, \alpha} \left( 0 \right) e^{ik_{p}x_{p}} \right)  \right|\psi_{i}; n_{\vec{k}, \alpha}  \right> \\
	&= -\frac{e}{m} \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar}{2\omega}} \left<\psi_{n}; n_{\vec{k}, \alpha} -1 \left| \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p} \sqrt{n_{\vec{k}, \alpha}} e^{ik_{p}x_{p}}  \right|\psi_{i}; n_{\vec{k}, \alpha} -1  \right> \\
	&= -\frac{e}{m} \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar n_{\vec{k}, \alpha}}{2\omega}} \left<\psi_{n} \left| e^{i\vec{k} \cdot \vec{p}} \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p} \right| \psi_{i}  \right> e^{-i \omega t}
.\end{align*}
\\
\\
...
\begin{align*}
	\left<n \left| \mathbf{H}_{int} \right|i  \right> &= \left<\psi_{n}; n_{\vec{k}, \alpha} +1 \left| - \frac{e}{mc} \vec{A} \cdot \vec{p} \right|\psi_{i};n_{\vec{k}, \alpha}  \right> \\
	\left<n \left| \mathbf{H}^{\left( emit \right) }_{int} \right|i  \right> &= - \frac{e}{mc} \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar c^2}{2\omega}} \left<\psi_{n}; n_{\vec{k}, \alpha} +1 \left| \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p} a^{\dagger}_{k, \alpha} \left( 0 \right) e^{-ik_{p}x_{p}} \right|\psi_{i}; n_{\vec{k}, \alpha}  \right> \\
	&= - \frac{e}{m} \frac{1}{\sqrt{V} } \sqrt{\frac{\hbar \left( n_{\vec{k}, \alpha} + 1 \right) }{2\omega}}  \left<\psi_{n} \left| e^{-i\vec{k} \cdot \vec{r}} \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p} \right| \psi_{i}  \right> e^{i \omega t} 
.\end{align*}
\\
\\
These give the same result as our earlier guess to put an $\displaystyle n+1$ in the emission operator.

\section{Review of Radiation of Photons}

In the previous section, we derived the same formulas for matrix elements that we had earlier used to study decay of Hydrogen atom states with no applied EM field, that is zero photons in the initial state.

\begin{equation}
	\Gamma_{i \to n} = \frac{\left( \pi \right)^2 e^2 }{m^2 \omega V} \left|\left< \phi \left|e^{-i\vec{k} \cdot \vec{r}} \hat{\epsilon} \cdot \vec{p} \right| \phi_{i} \right>\right|^2 \delta \left( E_{n} - E_{i} + \hbar \omega \right) 
\end{equation}
\\
\\
...
\\
\begin{equation}
	\Gamma_{tot} = \frac{e^2 \left( E_{i} - E_{n} \right) }{2 \pi \hbar^2 m^2 c^3} \sum_{\lambda} \int d \Omega_{p} \left| \left<\phi_{n} \left| e^{ -i\vec{k} \cdot \vec{r} } \hat{\epsilon}^{\left( \lambda \right) } \cdot \vec{p}_{w} \right| \phi_{i}  \right> \right|^2
\end{equation}
\\
\\
The quantity $\vec{K} \cdot \vec{p}$ is typically small for atomic transitions
\\
\\
\begin{align*}
	E_{\gamma} &= pc = \hbar kc \approx \frac{1}{2} \alpha^2 mc^2 \\
	r &\approx a_0 = \frac{\hbar}{\alpha mc} \\
	kr &\approx \frac{1}{2} \frac{\alpha^2 mc}{\hbar} \frac{\hbar}{\alpha mc} = \frac{\alpha}{2}
.\end{align*}
\\
\\
Note that we have to take the full binding energy as the energy difference between states so almost all transitions will have $kr$ smaller than this estimate. This makes  $\vec{k} \cdot \vec{r}$ an excellent parameter in which to expand decay rate formulas. The approximation that $e^{ -i\vec{k} \cdot\vec{r} } \approx 1$ is a very good one and is called the electric dipole or E1 approximation. We previously derived the E1 selection rules.
\\
\\
\begin{align*}
	\Delta \ell &= \pm_1 \\
...
\\
\begin{align*}
	\Gamma_{tot} &= \frac{e^2 \left( E_{i} - E_{n} \right) }{2 \pi \hbar^2 m^2 c^{3}} \sum_{\lambda}  \int d \Omega_{\gamma} \left| \left<\phi_{n} \left| e^{ -i\vec{k} \cdot \vec{r} } \hat{\epsilon}^{\left( \lambda \right) } \cdot \vec{p}_{e} \right| \phi_{i} \right> \right|^2 \\
	&\approx \frac{\alpha \omega^3_{in}}{2 \pi c^2} \sum_{\lambda} \int  d\Omega_{\gamma} \left| \sqrt{\frac{4\pi}{3}} \int_{0}^{\infty} r^3 dr R^{*}_{n_{n}\ell_{i}} R_{n_{i}m_{n}} \int  \, d\Omega Y^{*}_{\ell_{n} m_{n}} \left( \epsilon_{z}Y_{10} + \frac{-\epsilon_{x} + ieps_{y}}{\sqrt{2} } Y_{11} + \frac{\epsilon_{x} + i \epsilon_{y}}{\sqrt{2} } Y_{1-1} \right) Y_{\ell_{i}m_{i}}   \right|^2
.\end{align*}
\\
\\
...
\\
\begin{equation}
    \Gamma_{tot} = \frac{4 \alpha \omega^3_{in}}{3c^2} \left| \vec{r}_{ni} \right|^2
\end{equation}
\\
\\
Here $\vec{r}_{ni}$ is the matrix element of the coordinate vector between final and initial states.
\\
\\
For single electron atoms, we can sum over the final states with different $m$ and get a formula that only requires us to do a radial integral.
\\
\\
 \begin{equation}
	 \Gamma_{tot} = \frac{4 \alpha \omega^3_{in}}{3c^2} \left\{
	 \begin{array}{l}
		 \frac{\ell + 1}{2\ell +1} \\
		 \frac{\ell}{2\ell +1}
	 \end{array}
	 \right\}
	 \left| \int_{0}^{\infty} R^{*}_{n' \ell'} R_{n \ell } r^3  \, dr  \right|^2 \qquad \text{ for } \qquad \ell' = \begin{cases}
	     \ell + 1 \\
	     \ell - 1
	 \end{cases}
\end{equation}
...
\section{Black Body Radiation Spectrum}
We are in a position to fairly easily calculate the spectrum of Black Body radiation. Assume there is a cavity with a radiation field on the inside and that the field interacts with the atoms of the cavity. Assume thermal equilibrium is reached. Let's take two atomic states that can make transitions to each other: $A \to B + \gamma \text{ and } B + \gamma \to A$. From statistical mechanics, we have
\\
\\
\begin{equation}
    \frac{N_{B}}{N_{A}} = \frac{e^{ -E_{b} / kT }}{e^{ -E_{A} / kT }} = e^{ \hbar \omega / kT }
\end{equation}
\\
\\
...
\\
\begin{align*}
	N_{B}\Gamma_{absorb} &= N_{A}\Gamma_{emit} \\
	\frac{N_{B}}{N_{A}} &= \frac{\Gamma_{emit}}{\bamma_{absorb}}
.\end{align*}
\\
\\
We have previously calculated the emission and absorption rates. We can calculate the ration between th3e emission and absorption rates per atom:
\\
\\
\begin{equation}
	\frac{N_{B}}{N_{A}} = \frac{\Gamma_{emit}}{\Gamma_absorb} = \frac{\left( n_{\vec{k}, \alpha} + 1  \right) \left| \sum_{i} \left<B \left| e^{ -i\vec{k} \cdot \vec{r}_{i} } \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p}_{i} \right|A  \right>  \right|^2  }{n_{\vec{k}, \alpha} \left| \sum_{i} \left<A \left| e^{ i\vec{k} \cdot \vec{r}_{i} } \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p}_{i} \right|B  \right>  \right|^2 }
\end{equation}
\\
\\
wherer the sum is over atomic electrons. The matrix elements are closely related.
\\
\\
\begin{equation}
    \left<B \left| e^{ -i\vec{k} \cdot \vec{r}_{i} } \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p}_{i} \right|A  \right> = \left<A \left| \vec{p}_{i} \cdot \hat{\epsilon}^{\left( \alpha \right) }e^{ i\vec{k} \cdot \vec{r}_{i} } \right| B  \right>^{*} = \left<A \left| e^{ i\vec{k} \cdot \vec{r}_{i} } \hat{\epsilon}^{\left( \alpha \right) } \cdot \vec{p}_{i} \right| B \right>^{*}
\end{equation}
\\
\\
We have used the fact that $\vec{k} \cdot \epsilon = 0$. The two matrix elements are simple complec conjugates of each other so that when we take the absolute square, they are the same. Therefore, we may cancel them.
\\
\\
\begin{align*}
	\frac{N_{B}}{N_{A}} &= \frac{\left( n_{\vec{k}, \alpha} +1 \right) }{n_{\vec{k}, \alpha}} = e^{ \hbar \omega / kT } \\
	1 &= n_{\vec{k}, \alpha} \left( e^{ \hbar \omega / kT } - 1 \right) \\
	n_{\vec{k}, \alpha} &= \frac{1}{e^{ \hbar \omega / kT -1 }}
.\end{align*}
\\
\\
Now suppose the walls of the cavity are black so that they emit and absorb photons at any energy. Then the result for the number of photons above is true for all the radiation modes of the cavity. The energy in the frequency interval $\left( \omega, \omega + d \omega \right) $ per unit volume can be calculated by multiplying the number of photons by the energy per photon times the number of modes in that frequency interval and dividing by the volume of the cavity.
\\
\\
\begin{align*}
	U \left( \omega \right) d \omega &= \frac{\hbar \omega}{e^{ \hbar \omega / kT } -1}2 \left( \frac{L}{2 \pi} \right)^3 4 \pi k^2 dk \frac{1}{L^3} \\
	U \left( \omega \right) &= 8\pi \frac{\hbar \omega}{e^{ \hbar \omega / kT } - 1} \left( \frac{1}{2\pi} \right)^3 k^2 \frac{dk}{d\omega} \\  U \left( \omega \right) &= \frac{8\pi \hbar}{c^{3}} \left( \frac{\omega}{2\pi} \right)^3 \frac{1}{e^{ \hbar \omega / kT }- 1} \\
	U \left( \nu \right) &= U \left( \omega \right) \frac{d\omega}{d \nu} = \frac{8\pi}{c^{3}} \frac{h \nu^3}{e^{ \hbar \omega / kT } - 1}
.\end{align*}
\\
\\
