...
\begin{align*}
	...
	\Delta E^{(obs)}_{n} &= \frac{2 \alpha \hbar}{3\pi m^2 c^2} \int_{0}^{\omega_{cut-off}} \left( \sum_{j} \frac{\omega}{\left( \omega_{nj} - \omega \right) \left| \left<n \left| e^{i \vec{k} \cdot \vec{x}}\vec{p} \right|j  \right> \right|^2 + \left<n \left| p^2 \right|n  \right>  }  \right)   \, d\omega
.\end{align*}
