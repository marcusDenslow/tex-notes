...
% TODO: add Feynman diagram image
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.6\textwidth]{feynman-self-energy.png}
%\end{figure}
...
\section{Perturbation Theory Setup}
\begin{align*}
...
\section{Energy Shift and Width}
...
\section{Evaluation of the Energy Shift}
\begin{align*}
...
\section{Free Electron Self Energy}
...
For a non-relativisitc free electron the energy $\displaystyle \frac{p^2}{2m}$ decreases as the mass of the electron increases, so the negative sign corresponds to a positive shift in the electron's mass, and hence an increase in the real energy of the electron. Later, we will think of this as a renormalization of the electron's mass. The electron starts off with some bare mass. The self-energy due to the interaction of the electron's charge with its own radiation field increases the mass to what is observed.
\\
\\
...
\section{Commutator Simplification}
...
.\end{align*}
\section{Relativistic Correction and Combined Result}
...
\end{equation}
The dependence on $\displaystyle \omega_{min}$ cancels. In this calculation, the $\displaystyle mc^2$ in the log is the outcome of the relativistic calculation, not the cutoff. The electric dipole approximation is even pretty good since we did not need to go up to large photon energies non-relativistically and no E1 approximation is needed for the relativistic part. That's how we (and Bethe) got about the right answer.

\section{Physical Interpretation}
The Lamb shift splits the $\displaystyle 2S_{\frac{1}{2}}$ and $\displaystyle 2P_{\frac{1}{2}}$ states which are otherwise degenerate. Its origin is purely from field theory. The experimental measurement of the Lamb shift stimulated theorists to develop Quantum ElectroDynamics. The correction increases the energy of s states. One may think of the physical origin as the electron becoming less point-like as virtual photons are emitted and reabsorbed. Spreading the electron out a bit decreases the effect of being in the deepest part of the potential, right at the origin. Based on the energy shift, I estimate that the electron in the 2s state is spread out about 0.005 Angtroms, much more than the size of the nucleus.
\\
\\
The anomalous magnetic moment of the electron, $\displaystyle g - 2$, which can also be calculated in field theory, makes a small contribution to the Lamb shift.
