...
\begin{align*}
	...
	&= \frac{e^2}{2m^2V} \int \frac{V d^3 k}{\left( 2\pi \right)^3 } \, \sum_{\alpha} \sum_{j}  \frac{\left| \left<n \left| e^{i \vec{k} \cdot \vec{x}} \vec{p} \cdot \hat{\epsilon}^{(\alpha)} \right|j  \right> \right|^2 }{\hbar \omega \left( \omega_{nj}- \omega \right) }
.\end{align*}
