...
\\
\\
Pick $\displaystyle \hat{\epsilon}^{\left( 1 \right)' }$ to be in the scattering plane (x-z) defined as the plane containing both $\displaystyle \vec{k} \text{ and } \vec{k}' \text{ and } \hat{\epsilon}^{\left( 2 \right)'}$ to be perpendicular to the scattering plane. $\displaystyle \hat{\epsilon}^{\left( 1 \right)'} \text{ is then at an angle. } \theta \text{ to the x-axis } \hat{\epsilon^{\left( 3 \right)'}}$ is
